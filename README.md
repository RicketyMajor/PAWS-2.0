...
- **Etapa 22** (Completada): Notificaciones visuales inteligentes y confirmación de lectura (Visto), sistema de contadores ocultos con subqueries en backend, callbacks de agregación de badges para comunicación padre-hijo en Flutter, marcación automática de chats al entrar, renderizado selectivo de estado de lectura en último mensaje, 2 sistemas integrados: contadores virtuales de no leídos (MatchService), confirmación de lectura con lógica condicional triple
...
- **Etapa 22**: Notificaciones visuales (badges de contadores de no leídos), confirmación de lectura (Visto), sistema de callbacks para agregación de contadores, automarkAsRead en ChatScreen (integrada en [Fase-15](documentation/Fase-15.md) con sección "COMPLETADO EN ETAPA 22")
...
- **Contadores Virtuales de No Leídos (Etapa 22)**: MatchService.GetAdopterMatches() y GetRescuerMatches() ejecutan subquery COUNT(messages) WHERE match_id = X AND sender_id != userId AND is_read = false. Campo virtual UnreadCount en respuesta JSON (no persiste en tabla matches). Permite agregación O(n) en frontend sin queries adicionales. Backend-driven calculation evita inconsistencias.
- **Callbacks de Agregación de Badges (Etapa 22)**: AdopterMatchesScreen y RescuerChatsScreen suman todos los unreadCounts con .fold(0, (sum, m) => sum + m.unreadCount). Invocan callback onBadgeUpdate(totalCount) al padre MainLayoutScreen. Callback pattern permite comunicación padre-hijo sin BLoC. Actualización reactiva en NavigationBar con \_buildBadgedIcon() que renderiza círculo rojo con contador.
- **Marcación Automática al Entrar (Etapa 22)**: ChatScreen.\_ChatScreenState.initState() llama \_markChatAsRead(). ChatRepository.markAsRead(matchId) dispara POST /matches/:id/read. Backend ChatService.MarkAsRead() actualiza is_read = true en todos los mensajes WHERE match_id = X AND sender_id != currentUserId AND is_read = false. Silent operation, sin feedback visual al usuario.
- **Confirmación de Lectura Selectiva (Etapa 22)**: ChatBubble renderiza "Visto" solo si (isSeen && isMe) es true. ChatScreen calcula isSeen = (index == 0 && isMe && msg.isRead): (1) Índice 0 = último mensaje (reverse list), (2) isMe verifica ownership, (3) msg.isRead del servidor. Tres condiciones AND evitan "ruido visual". Desaparece cuando nuevo mensaje llega (index 0 cambia). Pequeño texto gris al final del bubble.
